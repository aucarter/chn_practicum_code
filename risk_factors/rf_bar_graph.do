...
// Set locals
    ...
    local outdir "$prefix/temp/wgodwin/chn/bar_chart"
    local indir "$prefix/temp/wgodwin/chn/bar_chart"
    cd "`outdir'"
...
keep if level == 3 | inlist(rei_id, 170, 125)
...
foreach loc in `location_ids' {
    ...
    get_outputs, topic(rei) measure_id(`measure_id') metric_id(1) year_id(`year') cause_id(all) rei_id(`risks') sex_id(`sex_id') age_group_id(1) location_id(`loc') gbd_round_id(`gbd_id') compare_version_id(`compare_version') clear
        ...
        get_outputs, topic(cause) measure_id(`measure_id') metric_id(1) year_id(`year') cause_id(294) sex_id(`sex_id') age_group_id(1) location_id(`loc') gbd_round_id(`gbd_id') compare_version_id(`compare_version') clear
        gen rei_id = .
        gen rei_name = "_none"
        ...
        ** reshape by cause for graph
        ...
        gen cause_order = .  
        replace cause_order = 1 if cause_name == "Neonatal preterm birth complications"
        ...
        replace cause_order = 9 if cause_name == "Meningitis"
        replace cause_order = 10 if cause_name == "Upper respiratory infections"
        replace cause_order = 11 if cause_name == "Sudden infant death syndrome"
        replace cause_order = 12 if cause_name == "Protein-energy malnutrition"
        // replace cause_order = 13 if cause_name == "Drug use disorders"
        // replace cause_order = 14 if cause_name == "Encephalitis"
        // replace cause_order = 15 if cause_name == "HIV/AIDS"
        // replace cause_order = 16 if cause_name == "Measles"
        // replace cause_order = 17 if cause_name == "Otitis media"

        drop if cause_order == . // TEMPORARY //
        ...
        ** replace missing with 0
        ...
        // }
        order sex rei* total* order val_1 val_2 val_3 val_4 val_5 val_6 val_7 val_8 val_9 val_10 val_11 val_12 // val_13 val_14 val_15 val_16 val_17

        export delimited using "`indir'/inputs/figure_3_`location'.csv", replace
        
        if `loc' == 6 {
            pdfstart using "`outdir'/Figure_B_`location'_deaths_attributed_to_level_3_risks.pdf"
        }
