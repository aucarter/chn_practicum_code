...
risk.dt <- get_outputs(topic="rei", location_id=prov.list, year_id=years, age_group_id=1, sex_id="all",
	measure_id=1, metric_id=1, rei_id = risk_ids, version="latest")
...
n <- length(rank$rei_name)/10
rank[, values:=rep(1:10, times=n)]
...
# Actually plot
pdf(paste0(root, "temp/", user, "/chn/rf_hmap_top10.pdf",sep=""),width=15,height=9)

for(year in years) {
	for(sx in sexes) {
		temp <- rank[year_id==year & sex==sx]
		title=paste("Top 10 under-5 risk factors by province (mortality rate per 100,000) for", year, ",", sx, sep=" ")

		print(ggplot(temp, aes(reorder(location_name, -mr_sum), reorder(rei_name, rate))) +
			geom_tile(aes(fill=-values)) +
			geom_text(aes(label=round(rate, 0))) +
			scale_fill_gradient(low="white", high="red", guide=FALSE) +
			labs(title=paste(title, sep=""),
				x="Province (in order of all-cause attributable mortality rate, high to low)",
				y="Risk name") +
